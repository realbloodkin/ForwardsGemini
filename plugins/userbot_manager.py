...
async def add_command_userbot(bot: Client, message: Message):
    ...
    if user_id not in bot.userbots: bot.userbots[user_id] = {}
    if 'cmd' in bot.userbots[user_id]:
        return await message.reply_text("Command userbot already exists. Use `/removeuserbot` first.")
    try:
        ...
        await status_msg.edit_text(f"✅ **Command Userbot added!**\n**Name:** {userbot_info.first_name}")
...
async def remove_command_userbot(bot: Client, message: Message):
    ...
    if user_id in bot.userbots and 'cmd' in bot.userbots[user_id]:
        if bot.userbots[user_id]['cmd'].is_connected:
            await bot.userbots[user_id]['cmd'].stop()
        del bot.userbots[user_id]['cmd']
        await db.update_configs(user_id, 'command_userbot_session', None)
        await message.reply_text("✅ Command userbot removed.")
    else:
        await message.reply_text("No command userbot found.")
